...
class Nfa[A](val startState: A, val transitions: Map[(A, Char), Set[A]], val finalStates: Set[A], val states: Set[A]) {
  ...
  def map[B](f: A => B) : Nfa[B] = {
    ...
    new Nfa(f(startState), newTransitions, newFinalStates, newStates)
  } // TODO implement map
  ...
  def next(state:A, c: Char): Set[A] = {
    ...
    epsilonStates.foreach(s => nextStates ++= next(s, c))
...
object Nfa {
  ...
  def fromPrenex(str: String): Nfa[Int] = {
    ...
    def fromTreeToNfa(tree: BTree[String]): Nfa[Int] = {
      ...
      new Nfa(0, transitions.toMap, finalStates.toSet, states.toSet)
    }

    val (syntaxTree, _) = fromPrenexRec(tokens, new BTree("", None, None))
    val s = syntaxTree.toString
    //println(s)
